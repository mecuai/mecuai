Cross-compiliation of MecuAI Core
===============================

MecuAI Core can be cross-compiled on Linux to all other supported host systems. This is done by changing
the `HOST` parameter when building the dependencies and then specifying another `--prefix` directory when building MecuAI.
...
Note that for WSL the MecuAI Core source path MUST be somewhere in the default mount file system, for
example /usr/src/mecuai, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
It's not possible to build the MecuAI Core code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the MecuAI Core source code).
