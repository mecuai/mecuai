...
PACKAGE_VERSION=$($SRC_PATH/src/mecuaid --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/mecuaid $DEB_BIN
cp $SRC_PATH/src/mecuai-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/mecuai-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/mecuaid.1 $DEB_MAN
cp $SRC_DOC/man/mecuai-cli.1 $DEB_MAN
cp $SRC_DOC/man/mecuai-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/mecuaid.bash-completion $DEB_CMP/mecuaid
cp $SRC_PATH/contrib/mecuai-cli.bash-completion $DEB_CMP/mecuai-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/mecuaid.1
gzip --best -n $DEB_MAN/mecuai-cli.1
gzip --best -n $DEB_MAN/mecuai-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/mecuaid $DEB_BIN/mecuai-cli
