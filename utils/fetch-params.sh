...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/MecuAIParams"
else
    PARAMS_DIR="$HOME/.mecuai-params"
fi
...
DOWNLOAD_URL="https://github.com/mecuai/mecuai/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with MECU_DISABLE_SOMETHING=1
MECU_DISABLE_WGET="${MECU_DISABLE_WGET:-}"
#MECU_DISABLE_IPFS="${MECU_DISABLE_IPFS:-}"
MECU_DISABLE_CURL="${MECU_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$MECU_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$MECU_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$MECU_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the MecuAI parameters!
...
main() {
    ...
    cat <<EOF
MECU - fetch-params.sh

This script will fetch the MecuAI SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common MecuAI SNARK parameters. Note that it is
