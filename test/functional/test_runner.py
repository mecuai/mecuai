...
def main():
    ...
    # args to be passed on always start with two mecuaies; tests are the remaining unknown args
    ...
    tmpdir = "%s/mecuai_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and mecuaid must all be enabled")
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, failfast=False, runs_ci, combined_logs_len=0):
    ...
    # Warn if mecuaid is already running (unix only)
    try:
        pidof_output = subprocess.check_output(["pidof", "mecuaid"])
        if not (pidof_output is None or pidof_output == b''):
            print("%sWARNING!%s There is already a mecuaid process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
...
class RPCCoverage():
    ...
    commands per `mecuai-cli help` (`rpc_interface.txt`).
