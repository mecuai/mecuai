...
const char * const BITCOIN_CONF_FILENAME = "mecuai.conf";
const char * const BITCOIN_PID_FILENAME = "mecuaid.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "mecuaid -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MecuAICore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MecuAICore
    // Mac: ~/Library/Application Support/MecuAICore
    // Unix: ~/.mecuaicore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MecuAICore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MecuAICore";
#else
    // Unix
    return pathRet / ".mecuaicore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty mecuai.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure MecuAI Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("MecuAI Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The MecuAI Core developers";
    }
