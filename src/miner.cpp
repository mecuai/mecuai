...
void static MecuAIMiner(const CChainParams& chainparams)
{
    LogPrintf("MecuAIMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mecuai-miner");
    ...
    CWallet * pWallet = NULL;
        ...
        if (!EnsureWalletIsAvailable(pWallet, false)) {
            LogPrintf("MecuAIMiner -- Wallet not available\n");
        }
    #endif
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("MecuAIMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            ...
            LogPrintf("MecuAIMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("MecuAIMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("MecuAIMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateMecuAIs(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&MecuAIMiner, boost::cref(chainparams)));
    }
    return(numCores);
}
