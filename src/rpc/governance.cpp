...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 MECUAI irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on mecuaicentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by smartnode configured in mecuai.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.pushKV("mecuai.conf", statusObj);
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by smartnode configured in mecuai.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* MecuAI features */
    { "mecuai",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "mecuai",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "mecuai",               "gobject",                &gobject,                {} },
    { "mecuai",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
