...
# mecuai_hash
RUN git clone https://github.com/mecuai/mecuai_hash
RUN cd mecuai_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} mecuai
RUN useradd -u ${USER_ID} -g mecuai -s /bin/bash -m -d /mecuai mecuai
...
RUN mkdir /mecuai-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /mecuai-src && \
